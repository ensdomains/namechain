...
contract MigrationErrorsTest is Test {
    ...
    function testFuzz_wrap(bytes calldata v) external pure {
        assertEq(WrappedErrorLib.unwrap(WrappedErrorLib.wrap(v)), v);
    }

    function testFuzz_encode(bytes calldata v) external pure {
        assertEq(WrappedErrorLib.decode(WrappedErrorLib.encode(v)), v);
    }

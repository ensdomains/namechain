...
import {
    ...
    BridgeRolesLib,
    BridgeEncoderLib
} from "~src/L1/bridge/L1BridgeController.sol";
import {
...
contract UnlockedMigrationControllerTest is NameWrapperFixture, ETHRegistryMixin, NameMixin {
    MockL1Bridge bridge;
    ...
    function _assertMigration(
        ...
        for (uint256 i; i < logs.length; ++i) {
            if (
                ...
                if (keccak256(bytes(td.label)) == keccak256(bytes(md.label))) {
                ...
                logs[i].topics[0] == keccak256("NameInjected(uint256,string)")
            ) {
                string memory label = abi.decode(logs[i].data, (string));
                if (keccak256(bytes(label)) == keccak256(bytes(md.label))) {
    ...
    function test_Revert_migrate_unwrapped_unregistered(bool toL1) external {
        ...
        migrationController.migrate(_makeData(NameCoder.ethName("test"), toL1));
        vm.stopPrank();
    }

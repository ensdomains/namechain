...
contract BridgeEncoderWrapper {
    ...
    function decodeRenewal(bytes memory message) external pure returns (uint256 tokenId, uint64 newExpiry) {
...
contract BridgeEncoderTest is Test {
    ...
    function testDecodeEjectionInvalidMessageType() public {
        ...
        bytes memory invalidMessage = abi.encode(uint256(BridgeMessageType.RENEWAL), dnsEncodedName, transferData);
    ...
    function testDecodeRenewalInvalidMessageType() public {
        ...
        // Manually encode with wrong message type to test custom error
        bytes memory invalidMessage = abi.encode(uint256(BridgeMessageType.EJECTION), tokenId, newExpiry);

...
import {
    ...
    IRegistry,
    NameCoder
} from "../../src/universalResolver/libraries/RegistryTraversalLib.sol";

contract TestRegistryTraversalLib is Test, ERC1155Holder {
    ...
    function test_readLabel_root() external pure {
        ...
        assertEq(RegistryTraversalLib.readLabel(name, 0), "");
    }
    function test_readLabel_eth() external pure {
        bytes memory name = NameCoder.encode("eth");
        assertEq(RegistryTraversalLib.readLabel(name, 0), "eth");
        assertEq(RegistryTraversalLib.readLabel(name, 4), ""); // 3eth
    }
    function test_readLabel_test_eth() external pure {
        bytes memory name = NameCoder.encode("test.eth");
        assertEq(RegistryTraversalLib.readLabel(name, 0), "test");
        assertEq(RegistryTraversalLib.readLabel(name, 5), "eth"); // 4test
        assertEq(RegistryTraversalLib.readLabel(name, 9), ""); // 4test3eth
    }

    function _readLabel(bytes memory name, uint256 offset) public pure returns (string memory) {
        return RegistryTraversalLib.readLabel(name, offset);
    }
    ...
    function _expectFindResolver(
        ...
        (, address resolver, bytes32 node, uint256 offset) = RegistryTraversalLib.findResolver(
            rootRegistry,
            name,
            0
        );
        ...
        assertEq(
            address(RegistryTraversalLib.findParentRegistry(rootRegistry, name, 0)),
            parentRegistry,
            "parent"
        );
        uint256 i;
        for (offset = 0; i < registries.length; i++) {
            assertEq(
                address(RegistryTraversalLib.findExactRegistry(rootRegistry, name, offset)),

...
contract TestL1EjectionController is Test, ERC1155Holder, RegistryRolesMixin, EnhancedAccessControl {
    ...
    function supportsInterface(bytes4 /*interfaceId*/ )
        public
        pure
        override(ERC1155Holder, EnhancedAccessControl)
        returns (bool)
    {
        return true;
    }
    ...
    function _createBatchEjectionData(
        ...
        require(
            l2Owners.length == l2Subregistries.length && l2Owners.length == l2Resolvers.length
                && l2Owners.length == expiryTimes.length && l2Owners.length == roleBitmaps.length,
            "Array lengths must match"
        );

        EjectionController.TransferData[] memory transferDataArray =
            new EjectionController.TransferData[](l2Owners.length);
    ...
    function test_eject_from_namechain_basic() public {
        ...
        ejectionController.completeEjectionFromNamechain(
            testLabel, user, subregistry, MOCK_RESOLVER, expiryTime, expectedRoles, ""
        );
    ...
    function test_Revert_updateExpiration_reduce_expiry() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IStandardRegistry.CannotReduceExpiration.selector, initialExpiry, newExpiry)
        );
        ejectionController.syncRenewal(tokenId, newExpiry);
    }
    ...
    function test_onERC1155BatchReceived() public {
        ...
        registry.register(
            testLabel1, address(this), registry, MOCK_RESOLVER, ROLE_SET_RESOLVER | ROLE_SET_SUBREGISTRY, expiryTime
        );
        registry.register(
            testLabel2, address(this), registry, MOCK_RESOLVER, ROLE_SET_RESOLVER | ROLE_SET_SUBREGISTRY, expiryTime
        );
        registry.register(
            testLabel3, address(this), registry, MOCK_RESOLVER, ROLE_SET_RESOLVER | ROLE_SET_SUBREGISTRY, expiryTime
        );
...
contract MockL1EjectionController is L1EjectionController {
    event MockNameEjectedToL2(
        uint256 tokenId, address l1Owner, address l1Subregistry, address l1Resolver, uint64 expires
    );
    event MockNameEjectedFromL2(
        string label, address l1Owner, address l1Subregistry, address l1Resolver, uint64 expires
    );
     ...
     */
    function _onEject(uint256[] memory tokenIds, EjectionController.TransferData[] memory transferDataArray)
        internal
        override
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            emit MockNameEjectedToL2(
                tokenIds[i], transferData.owner, transferData.subregistry, transferData.resolver, transferData.expires
            );
        }
    }
}

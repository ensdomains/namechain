...
contract TestLibLockedNames is Test {
    ...
    function test_freezeName_clears_resolver_when_fuse_not_set() public {
        ...
        assertEq(
            fusesFromEvent,
            uint16(LibLockedNames.FUSES_TO_BURN),
        ...
        assertTrue(
            (finalFuses & LibLockedNames.FUSES_TO_BURN) == LibLockedNames.FUSES_TO_BURN,
    ...
    function test_generateRoleBitmapsFromFuses_all_permissions() public pure {
        ...
        assertTrue(
            (tokenRoles & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER) != 0,
            "Token should have ROLE_SET_TOKEN_OBSERVER"
        );
        assertTrue(
            (tokenRoles & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER_ADMIN) != 0,
            "Token should have ROLE_SET_TOKEN_OBSERVER_ADMIN"
        );
        ...
        assertTrue(
            (tokenRoles & LibRegistryRoles.ROLE_RENEW_ADMIN) != 0,
            "Token should have ROLE_RENEW_ADMIN"
        );
        assertTrue(
            (tokenRoles & LibRegistryRoles.ROLE_SET_RESOLVER) != 0,
            "Token should have ROLE_SET_RESOLVER"
        );
        assertTrue(
            (tokenRoles & LibRegistryRoles.ROLE_SET_RESOLVER_ADMIN) != 0,
            "Token should have ROLE_SET_RESOLVER_ADMIN"
        );
        // Should include transfer role since CANNOT_TRANSFER is not set
        assertTrue(
            (tokenRoles & LibRegistryRoles.ROLE_CAN_TRANSFER_ADMIN) != 0,
            "Token should have ROLE_CAN_TRANSFER"
        );
        // Token should NEVER have registrar roles
        assertTrue(
            (tokenRoles & LibRegistryRoles.ROLE_REGISTRAR) == 0,
            "Token should NEVER have ROLE_REGISTRAR"
        );
        assertTrue(
            (tokenRoles & LibRegistryRoles.ROLE_REGISTRAR_ADMIN) == 0,
            "Token should NEVER have ROLE_REGISTRAR_ADMIN"
        );

        // SubRegistry should have registrar roles since subdomain creation is allowed
        assertTrue(
            (subRegistryRoles & LibRegistryRoles.ROLE_REGISTRAR) != 0,
            "SubRegistry should have ROLE_REGISTRAR"
        );
        assertTrue(
            (subRegistryRoles & LibRegistryRoles.ROLE_REGISTRAR_ADMIN) != 0,
            "SubRegistry should have ROLE_REGISTRAR_ADMIN"
        );
        // SubRegistry should have renewal roles (always granted)
        assertTrue(
            (subRegistryRoles & LibRegistryRoles.ROLE_RENEW) != 0,
            "SubRegistry should have ROLE_RENEW"
        );
        assertTrue(
            (subRegistryRoles & LibRegistryRoles.ROLE_RENEW_ADMIN) != 0,
            "SubRegistry should have ROLE_RENEW_ADMIN"
        );
        // SubRegistry should NOT have token observer roles (those are only for tokenRoles)
        assertTrue(
            (subRegistryRoles & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER) == 0,
            "SubRegistry should NOT have ROLE_SET_TOKEN_OBSERVER"
        );
        assertTrue(
            (subRegistryRoles & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER_ADMIN) == 0,
            "SubRegistry should NOT have ROLE_SET_TOKEN_OBSERVER_ADMIN"
        );
    }
    ...
    function test_generateRoleBitmapsFromFuses_no_extend_expiry() public pure {
        ...
        assertTrue(
            (tokenRoles & LibRegistryRoles.ROLE_RENEW) == 0,
    ...
    function test_generateRoleBitmapsFromFuses_cannot_burn_fuses_no_admin_roles() public pure {
        ...
        assertTrue(
            (tokenRoles & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER_ADMIN) != 0,
            "Token should have ROLE_SET_TOKEN_OBSERVER_ADMIN (not affected by CANNOT_BURN_FUSES)"
        );
    ...
    function test_generateRoleBitmapsFromFuses_cannot_burn_fuses_with_restrictions() public pure {
        ...
        uint256 expectedTokenRoles = LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER |
        ...
        uint256 expectedSubRegistryRoles = LibRegistryRoles.ROLE_RENEW |
            LibRegistryRoles.ROLE_RENEW_ADMIN;
        assertEq(
            subRegistryRoles,
    ...
    function test_generateRoleBitmapsFromFuses_always_includes_token_observer_roles() public pure {
        ...
        (uint256 tokenRoles1, ) = LibLockedNames.generateRoleBitmapsFromFuses(minimalFuses);
        assertTrue(
            (tokenRoles1 & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER) != 0,
            "Should have ROLE_SET_TOKEN_OBSERVER with minimal fuses"
        );
        assertTrue(
            (tokenRoles1 & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER_ADMIN) != 0,
            "Should have ROLE_SET_TOKEN_OBSERVER_ADMIN with minimal fuses"
        );

        // Test with all restrictive fuses except CANNOT_BURN_FUSES
        uint32 restrictiveFuses = CANNOT_UNWRAP |
            IS_DOT_ETH |
            CANNOT_SET_RESOLVER |
            CANNOT_CREATE_SUBDOMAIN;
        (uint256 tokenRoles2, ) = LibLockedNames.generateRoleBitmapsFromFuses(restrictiveFuses);
        assertTrue(
            (tokenRoles2 & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER) != 0,
            "Should have ROLE_SET_TOKEN_OBSERVER with restrictive fuses"
        );
        assertTrue(
            (tokenRoles2 & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER_ADMIN) != 0,
            "Should have ROLE_SET_TOKEN_OBSERVER_ADMIN with restrictive fuses"
        );
        ...
        (uint256 tokenRoles3, ) = LibLockedNames.generateRoleBitmapsFromFuses(frozenFuses);
        assertTrue(
            (tokenRoles3 & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER) != 0,
            "Should have ROLE_SET_TOKEN_OBSERVER even with CANNOT_BURN_FUSES"
        );
        assertTrue(
            (tokenRoles3 & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER_ADMIN) != 0,
            "Should have ROLE_SET_TOKEN_OBSERVER_ADMIN (not affected by CANNOT_BURN_FUSES)"
        );

        // Test with all permission fuses
        uint32 allPermissionFuses = CANNOT_UNWRAP | IS_DOT_ETH | CAN_EXTEND_EXPIRY;
        (uint256 tokenRoles4, ) = LibLockedNames.generateRoleBitmapsFromFuses(allPermissionFuses);
        assertTrue(
            (tokenRoles4 & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER) != 0,
            "Should have ROLE_SET_TOKEN_OBSERVER with all permissions"
        );
        assertTrue(
            (tokenRoles4 & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER_ADMIN) != 0,
            "Should have ROLE_SET_TOKEN_OBSERVER_ADMIN with all permissions"
        );

        // Verify token observer roles are never set on subRegistryRoles
        uint32 anyFuses = CANNOT_UNWRAP | IS_DOT_ETH | CAN_EXTEND_EXPIRY;
        (, uint256 subRegistryRoles) = LibLockedNames.generateRoleBitmapsFromFuses(anyFuses);
        assertTrue(
            (subRegistryRoles & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER) == 0,
            "SubRegistry should NEVER have ROLE_SET_TOKEN_OBSERVER"
        );
        assertTrue(
            (subRegistryRoles & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER_ADMIN) == 0,
            "SubRegistry should NEVER have ROLE_SET_TOKEN_OBSERVER_ADMIN"
        );
    }
    ...
    function test_generateRoleBitmapsFromFuses_fuses_control_renewal_roles() public pure {
        ...
        (uint256 tokenRoles1, ) = LibLockedNames.generateRoleBitmapsFromFuses(fusesWithExpiry);
        assertTrue(
            (tokenRoles1 & LibRegistryRoles.ROLE_RENEW) != 0,
            "Should have ROLE_RENEW when CAN_EXTEND_EXPIRY is set"
        );
        assertTrue(
            (tokenRoles1 & LibRegistryRoles.ROLE_RENEW_ADMIN) != 0,
            "Should have ROLE_RENEW_ADMIN when CAN_EXTEND_EXPIRY is set"
        );

        // Test without CAN_EXTEND_EXPIRY (should NOT have renewal roles)
        (uint256 tokenRoles2, ) = LibLockedNames.generateRoleBitmapsFromFuses(fusesWithoutExpiry);
        assertTrue(
            (tokenRoles2 & LibRegistryRoles.ROLE_RENEW) == 0,
            "Should NOT have ROLE_RENEW without CAN_EXTEND_EXPIRY"
        );
        assertTrue(
            (tokenRoles2 & LibRegistryRoles.ROLE_RENEW_ADMIN) == 0,
        ...
        assertTrue(
            (tokenRoles1 & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER) != 0,
            "Should have ROLE_SET_TOKEN_OBSERVER with CAN_EXTEND_EXPIRY"
        );
        assertTrue(
            (tokenRoles1 & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER_ADMIN) != 0,
            "Should have ROLE_SET_TOKEN_OBSERVER_ADMIN with CAN_EXTEND_EXPIRY"
        );
        assertTrue(
            (tokenRoles2 & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER) != 0,
            "Should have ROLE_SET_TOKEN_OBSERVER without CAN_EXTEND_EXPIRY"
        );
        assertTrue(
            (tokenRoles2 & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER_ADMIN) != 0,
            "Should have ROLE_SET_TOKEN_OBSERVER_ADMIN without CAN_EXTEND_EXPIRY"
        );
    }
    ...
    function test_generateRoleBitmapsFromFuses_fuses_consistency() public pure {
        ...
        (uint256 tokenRoles1, uint256 subRegistryRoles1) = LibLockedNames
            .generateRoleBitmapsFromFuses(fuses);
        (uint256 tokenRoles2, uint256 subRegistryRoles2) = LibLockedNames
            .generateRoleBitmapsFromFuses(fuses);
        ...
        assertTrue(
            (tokenRoles1 & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER) != 0,
            "Should have ROLE_SET_TOKEN_OBSERVER"
        );
        assertTrue(
            (tokenRoles1 & LibRegistryRoles.ROLE_SET_TOKEN_OBSERVER_ADMIN) != 0,
            "Should have ROLE_SET_TOKEN_OBSERVER_ADMIN"
        );
        assertTrue(
            (tokenRoles1 & LibRegistryRoles.ROLE_SET_RESOLVER) != 0,
            "Should have ROLE_SET_RESOLVER"
        );
        assertTrue(
            (tokenRoles1 & LibRegistryRoles.ROLE_SET_RESOLVER_ADMIN) != 0,
    ...
    function test_generateRoleBitmapsFromFuses_frozen_fuses_behavior() public pure {
        ...
        (uint256 tokenRoles, ) = LibLockedNames.generateRoleBitmapsFromFuses(frozenFuses);
    ...
    function test_generateRoleBitmapsFromFuses_without_can_extend_expiry() public pure {
        ...
        (uint256 tokenRoles, ) = LibLockedNames.generateRoleBitmapsFromFuses(fusesWithoutExpiry);
    ...
    function test_generateRoleBitmapsFromFuses_all_restrictive_fuses() public pure {
        ...
        assertTrue(
            (tokenRoles & LibRegistryRoles.ROLE_CAN_TRANSFER_ADMIN) == 0,
            "Token should NOT have ROLE_CAN_TRANSFER"
        );
        assertTrue(
            (tokenRoles & LibRegistryRoles.ROLE_REGISTRAR) == 0,
            "Token should NOT have ROLE_REGISTRAR"
        ...
        assertEq(
            tokenRoles,
            expectedTokenRoles,
            "Token should only have token observer roles with all restrictive fuses"
        );
        uint256 expectedSubRegistryRoles = LibRegistryRoles.ROLE_RENEW |
            LibRegistryRoles.ROLE_RENEW_ADMIN;
        assertEq(
    ...
    function test_generateRoleBitmapsFromFuses_transfer_role_consistency() public pure {
        ...
        (uint256 tokenRoles1, ) = LibLockedNames.generateRoleBitmapsFromFuses(
            fusesWithCannotTransfer
        );
        assertTrue(
            (tokenRoles1 & LibRegistryRoles.ROLE_CAN_TRANSFER_ADMIN) == 0,
        ...
        (uint256 tokenRoles2, ) = LibLockedNames.generateRoleBitmapsFromFuses(
            fusesWithoutCannotTransfer
        );
        assertTrue(
            (tokenRoles2 & LibRegistryRoles.ROLE_CAN_TRANSFER_ADMIN) != 0,
        ...
        uint32 fusesWithOtherRestrictions = CANNOT_UNWRAP |
            ...
            CANNOT_SET_RESOLVER |
            CANNOT_CREATE_SUBDOMAIN;
        (uint256 tokenRoles3, ) = LibLockedNames.generateRoleBitmapsFromFuses(
            fusesWithOtherRestrictions
        );
        assertTrue(
            (tokenRoles3 & LibRegistryRoles.ROLE_CAN_TRANSFER_ADMIN) != 0,
        ...
        uint32 fusesWithCannotTransferAndOthers = CANNOT_UNWRAP |
            ...
            CANNOT_TRANSFER |
            CANNOT_SET_RESOLVER;
        (uint256 tokenRoles4, ) = LibLockedNames.generateRoleBitmapsFromFuses(
            fusesWithCannotTransferAndOthers
        );
        assertTrue(
            (tokenRoles4 & LibRegistryRoles.ROLE_CAN_TRANSFER_ADMIN) == 0,

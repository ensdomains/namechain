...
contract TestL2EjectionController is Test, ERC1155Holder, RegistryRolesMixin {
    ...
    function _createBatchEjectionData(
        ...
        require(
            labels.length == owners.length && labels.length == subregistries.length && labels.length == resolvers.length
                && labels.length == expiryTimes.length && labels.length == roleBitmaps.length,
            "Array lengths must match"
        );

        EjectionController.TransferData[] memory transferDataArray =
            new EjectionController.TransferData[](labels.length);
    ...
    function test_eject_flow_via_transfer() public {
        ...
        bytes memory ejectionData =
            _createEjectionData(label, l1Owner, l1Subregistry, l1Resolver, expiryTime, roleBitmap);
    ...
    function _verifyMigrationResults(
        uint256 _tokenId,
    ...
    function _verifyMigrationEvent(uint256 _tokenId, uint256 expectedRoleBitmap) internal {
        ...
        for (uint256 i = 0; i < logs.length; i++) {
            if (
                logs[i].emitter == address(controller) && logs[i].topics[0] == eventSig && logs[i].topics.length > 1
                ...
                if (logs[i].data.length > 0) {
                    (
                        address emittedL2Owner,
                        address emittedL2Subregistry,
                        address emittedL2Resolver,
                        uint256 emittedRoleBitmap
    ...
    function test_onERC1155BatchReceived() public {
        ...
        bytes memory batchData =
            _createBatchEjectionData(labels, owners, subregistries, resolvers, expiries, roleBitmaps);
        ...
        for (uint256 i = 0; i < logs.length; i++) {
            if (logs[i].emitter == address(controller) && logs[i].topics[0] == expectedSig) {
                ejectionEventsCount++;
            }
        }
    ...
    function test_onRenew_emitsEvent() public {
        ...
        for (uint256 i = 0; i < logs.length; i++) {
            if (logs[i].emitter == address(controller) && logs[i].topics[0] == eventSig) {
                ...
                // Only decode data if there is data to decode
                if (logs[i].data.length > 0) {
                    (uint64 emittedExpiry, address emittedRenewer) = abi.decode(logs[i].data, (uint64, address));
...
contract MockL2EjectionController is L2EjectionController {
    ...
    event MockNameMigratedFromL1(
        uint256 indexed tokenId, address l2Owner, address l2Subregistry, address l2Resolver, uint256 roleBitmap
    );

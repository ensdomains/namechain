...
contract TestL1UnlockedMigrationController is Test, ERC1155Holder, ERC721Holder {
    ...
    function _assertL1MigratorEvent(
        ...
        for (uint256 i = 0; i < entries.length; i++) {
            if (
                entries[i].emitter == address(realL1EjectionController) &&
        ...
        assertTrue(
            foundMigratorEvent,
            string(abi.encodePacked("NameEjectedToL1 event not found for label: ", expectedLabel))
        );
    }
    ...
    function test_constructor() public view {
        ...
        assertEq(
            address(migrationController.L1_EJECTION_CONTROLLER()),
            address(realL1EjectionController)
        );
    ...
    function test_Revert_migrateUnwrappedEthName_wrong_caller() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                L1UnlockedMigrationController.UnauthorizedCaller.selector,
                address(this)
            )
        );
    ...
    function test_Revert_migrateWrappedEthName_single_locked() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(L1UnlockedMigrationController.MigrationNotSupported.selector)
        );
        vm.prank(user);
        nameWrapper.safeTransferFrom(user, address(migrationController), testTokenId, 1, data);
    }

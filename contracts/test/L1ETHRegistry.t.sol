...
contract TestL1ETHRegistry is Test, ERC1155Holder {
    ...
    function test_eject_from_l2_unlocked() public {
        uint256 expectedId =
            uint256(keccak256("test") & 0xfffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff8);

        uint256 tokenId = registry.ejectFromL2("test", address(this), registry, 0, uint64(block.timestamp) + 86400);
    ...
    function test_eject_from_l2_locked() public {
        ...
        uint256 expectedId =
            uint256(keccak256("test") & 0xfffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff8) | flags;

        uint256 tokenId = registry.ejectFromL2("test", address(this), registry, flags, uint64(block.timestamp) + 86400);
        vm.assertEq(tokenId, expectedId);
    }

    function test_eject_from_l2_emits_events() public {
        vm.recordLogs();
        registry.ejectFromL2("test", address(this), registry, 0, uint64(block.timestamp) + 86400);

        Vm.Log[] memory entries = vm.getRecordedLogs();
        // There are 4 events: TransferSingle, onERC1155Received callback, SubregistryUpdate, NewSubname, NameEjected
    ...
    function test_renew() public {
        uint256 tokenId = registry.ejectFromL2("test", address(this), registry, 0, uint64(block.timestamp) + 100);
    ...
    function test_Revert_renew_reduce_expiry() public {
        uint256 tokenId = registry.ejectFromL2("test", address(this), registry, 0, uint64(block.timestamp) + 200);
    ...
    function test_Revert_cannot_relinquish_if_not_owner() public {
        uint256 tokenId = registry.ejectFromL2("test", address(1), registry, 0, uint64(block.timestamp) + 86400);
    ...
    function test_fallbackResolver_when_resolver_not_set() public {
        ...
        registry.ejectFromL2("test", address(this), registry, 0, uint64(block.timestamp) + 100);
    ...
    function test_expired_name_can_be_reejected() public {
        ...
        uint256 newTokenId = registry.ejectFromL2("test", address(1), registry, 0, uint64(block.timestamp) + 100);
    ...
    function test_Revert_set_token_observer_if_not_owner() public {
        uint256 tokenId = registry.ejectFromL2("test", address(1), registry, 0, uint64(block.timestamp) + 100);
    ...
    function test_eject_from_l2_replace_existing() public {
        // Eject a name first time
        uint256 tokenId = registry.ejectFromL2("test", address(1), registry, 0, uint64(block.timestamp) + 100);
        assertEq(registry.ownerOf(tokenId), address(1));

        // Eject the same name to a different owner - should burn and mint to new owner
        registry.ejectFromL2("test", address(2), registry, 0, uint64(block.timestamp) + 200);
    ...
    function test_eject_from_l2_different_flags() public {
        ...
        uint96 newFlags = registry.FLAG_SUBREGISTRY_LOCKED();
        uint256 newTokenId =
            registry.ejectFromL2("test", address(this), registry, newFlags, uint64(block.timestamp) + 100);
    ...
    function test_cannot_unlock_locked_flags() public {
        ...
        uint96 initialFlags = registry.FLAG_FLAGS_LOCKED();
        uint256 tokenId =
            registry.ejectFromL2("test", address(this), registry, initialFlags, uint64(block.timestamp) + 100);
    ...
    function test_ejected_name_details() public {
        ...
        // Eject the name
        uint256 tokenId = registry.ejectFromL2(label, owner, IRegistry(registryAddr), flags, expires);
        ...
        assertEq(storedFlags, flags);
        assertEq(address(registry.getSubregistry(label)), registryAddr);

        // Check resolver behavior
        assertEq(registry.getResolver(label), registry.fallbackResolver());

        // Set up a different test case without flags to test setting resolver
        string memory label2 = "unlocked";
        uint256 unlockedTokenId = registry.ejectFromL2(label2, owner, IRegistry(registryAddr), 0, expires);

        // Try to set a resolver for the unlocked name - should succeed
        vm.startPrank(owner);
        address resolverAddr = address(0x5678);
        registry.setResolver(unlockedTokenId, resolverAddr);
        assertEq(registry.getResolver(label2), resolverAddr);
    ...
    function test_migrateToL2_with_locked_name() public {
        ...
        uint96 flags = registry.FLAG_SUBREGISTRY_LOCKED() | registry.FLAG_RESOLVER_LOCKED();
        uint256 tokenId = registry.ejectFromL2("test", address(this), registry, flags, uint64(block.timestamp) + 100);
    ...
    function test_multiple_observers() public {
        ...
        uint256 tokenId1 = registry.ejectFromL2("name1", address(this), registry, 0, uint64(block.timestamp) + 100);
        uint256 tokenId2 = registry.ejectFromL2("name2", address(this), registry, 0, uint64(block.timestamp) + 100);

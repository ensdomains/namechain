...
async function deployResolverWithRecords(
  ...
  if (records.description) {
    await resolver.write.setText(["description", records.description], {
      account,
    });
  }
...
async function getParentWithSubregistry(
  ...
  data: NonNullable<Awaited<ReturnType<typeof traverseL2Registry>>>;
  ...
  for (const name of names) {
    ...
    if (isSecondLevel) {
      ...
      try {
        ...
        const l1Owner = await env.l1.contracts.ETHRegistry.read.ownerOf([
          tokenId,
        ]);
...
export async function createSubname(
  ...
  for (let i = parts.length - 3; i >= 0; i--) {
    ...
    // Check if expired
    const currentTime = await env.l2.client.getBlock().then((b) => b.timestamp);
...
export async function registerTestNames(
  ...
  const currentTimestamp = await env.l2.client.getBlock().then((b) => b.timestamp);
  ...
  for (const label of labels) {
    ...
    if (shouldTrackGas) await trackGas("deployDedicatedResolver", resolver.deploymentReceipt);
    ...
    const registerTx = await env.waitFor(env.l2.contracts.ETHRegistry.write.register(
      [
        label,
        account.address,
        zeroAddress,
...
export async function reregisterName(
  ...
  console.log(`\nCurrent onchain timestamp: ${new Date(Number(await env.l2.client.getBlock().then((b) => b.timestamp)) * 1000).toISOString()}`);
  console.log(`\nCurrent onchain expiry: ${new Date(Number(initialExpiry) * 1000).toISOString()}`);

  // Step 2: Verify name is available for re-registration
  const isAvailable = await env.l2.contracts.ETHRegistrar.read.isAvailable([
    label,
  ]);
  ...
  const [, reregisteredEntry] =
    await env.l2.contracts.ETHRegistry.read.getNameData([label]);
  console.log(
    `New expiry: ${new Date(Number(reregisteredEntry.expiry) * 1000).toISOString()}`,
  );

  if (reregisteredEntry.expiry <= initialExpiry) {
    throw new Error(
      `Re-registration failed: new expiry (${reregisteredEntry.expiry}) should be greater than initial expiry (${initialExpiry})`,
    );
  }

  console.log(
    `✓ Re-registration successful! Expiry extended from ${initialExpiry} to ${reregisteredEntry.expiry}`,
  );
}
...
export async function testNames(env: CrossChainEnvironment) {
  ...
  const renewTx = await env.waitFor(env.l2.contracts.ETHRegistrar.write.renew(
    [label, duration, paymentToken, referrer],
    { account: env.namedAccounts.owner },
  ));
  ...
  const [changeRoleTokenId] =
    await env.l2.contracts.ETHRegistry.read.getNameData(["changerole"]);
  const grantTx = await env.waitFor(env.l2.contracts.ETHRegistry.write.grantRoles(
    [
      changeRoleTokenId,
  ...
  const revokeTx = await env.waitFor(env.l2.contracts.ETHRegistry.write.revokeRoles(
    [
      changeRoleTokenId,

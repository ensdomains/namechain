...
contract UnlockedMigrationController is IERC1155Receiver, IERC721Receiver, ERC165, Ownable {
    ...
    function migrate(Data[] memory mds) public {
        uint256 wrapped;
        ...
        for (uint256 i; i < mds.length; ++i) {
            ...
            if (NAME_WRAPPER.isWrapped(NameUtils.ETH_NODE, labelHash)) {
                uint256 id = uint256(NameCoder.namehash(NameUtils.ETH_NODE, labelHash));
                // by construction, this is already .eth
                (, uint32 fuses, ) = NAME_WRAPPER.getData(id);
                if ((fuses & CANNOT_UNWRAP) != 0) {
                    revert MigrationErrors.NameIsLocked(NameUtils.appendETH(md.label));
                }
    ...
    function _finishMigration(Data memory md) internal {
        TransferData memory td = TransferData({
            name: NameUtils.appendETH(md.label),
            owner: md.owner,
        ...
        if (md.toL1) {
            ...
            L1_BRIDGE_CONTROLLER.BRIDGE().sendMessage(BridgeEncoder.encodeEjection(td));
        }
    }
}

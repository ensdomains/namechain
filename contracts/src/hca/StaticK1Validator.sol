...
import {MODULE_TYPE_VALIDATOR, VALIDATION_SUCCESS, VALIDATION_FAILED} from "nexus/types/Constants.sol";
...
contract StaticK1Validator is IValidator, IK1Validator, ERC7739Validator {
    ...
    function isValidSignatureWithSender(
        ...
        return
            _erc1271IsValidSignatureWithSender(
                sender,
                hash,
    ...
    function _erc1271IsValidSignatureNowCalldata(
        ...
        return
            _validateSignatureForOwner(getOwner(msg.sender), hash, signature);
    }
    ...
    function _fillSafeSenders(bytes calldata data) private {
        ...
        for (uint256 i; i < data.length / 20; i++) {
            _safeSenders.add(
                msg.sender,
                address(bytes20(data[20 * i:20 * (i + 1)]))
            );
        }
    }

...
interface IStandardRegistry is IRegistry {
    ...
    /// @notice Thrown when a name has expired.
    error NameExpired(uint256 tokenId);

    /// @notice Thrown when a name cannot be reduced in expiration.
    error CannotReduceExpiration(uint64 oldExpiration, uint64 newExpiration);

    /// @notice Thrown when a name cannot be set to a past expiration.
    ...
    /// @notice Fetches the expiry date of a subdomain.
    ///

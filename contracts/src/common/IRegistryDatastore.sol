...
interface IRegistryDatastore {
    ...
    event ResolverUpdate(address indexed registry, uint256 indexed id, address resolver, uint96 flags);
    ...
    function getResolver(address registry, uint256 id) external view returns (address resolver, uint96 flags);

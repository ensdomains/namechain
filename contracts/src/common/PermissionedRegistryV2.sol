...
contract PermissionedRegistryV2 is PermissionedRegistry {
     ...
     */
    function setResolverFactory(address _resolverFactory, address _resolverImplementation)
        external
    ...
    function deployResolver(string calldata label, address owner) external returns (address) {
        ...
        bytes memory initData = abi.encodeWithSelector(SingleNameResolver.initialize.selector, owner, namehash);
        ...
        address resolverAddress =
            VerifiableFactory(resolverFactory).deployProxy(resolverImplementation, uint256(salt), initData);
     ...
     */
    function setResolver(string calldata label, address resolver)
        external
        onlyNonExpiredTokenRoles(NameUtils.labelToCanonicalId(label), ROLE_SET_RESOLVER)
    {
        uint256 tokenId = NameUtils.labelToCanonicalId(label);
